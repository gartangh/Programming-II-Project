...
void GameMulti::Run()
{
	...
	while (!quit && context->GetState() <= 0)
	{
		...
		if (event.type == ALLEGRO_EVENT_KEY_DOWN)
		{
			...
			if (keycode == ALLEGRO_KEY_A || keycode == ALLEGRO_KEY_W ||
				keycode == ALLEGRO_KEY_Z || keycode == ALLEGRO_KEY_Q || keycode == ALLEGRO_KEY_D ||
				keycode == ALLEGRO_KEY_UP || keycode == ALLEGRO_KEY_LEFT || keycode == ALLEGRO_KEY_RIGHT ||
...
void GameMulti::MakeEntities()
{
	...
	ComponentSprite* cspr_ball = new ComponentSprite(Graphics::SPRITE_BALL, SLIME_1_INIT_X, 11, 738, 11, BALL_INIT_Y, 11, 288, 11);
	ball->Add(cspr_ball);
